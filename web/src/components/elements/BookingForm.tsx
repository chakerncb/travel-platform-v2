...
export default function BookingForm({ tour }: BookingFormProps) {
	...
	// Check if group size limit is exceeded
	const maxGroupSize = tour?.max_group_size || 20
	const isGroupSizeExceeded = totalParticipants > maxGroupSize
	...
	const availableDates = useMemo(() => {
		...
		// Generate next 30 days of available dates starting from tour start_date
		for (let i = 0; i < 30; i++) {
	...
	return (
		<>
			<div className="content-booking-form">
				...
				{/* Date Selection */}
				<div className="item-line-booking">
					<strong className="text-md-bold neutral-1000">Select Date:</strong>
					...
					<div className="box-tickets">
							...
							<div className="dropdown-quantity">
								<div className="dropdown">
									...
									<ul className="dropdown-menu dropdown-menu-light" aria-labelledby="dropdownTicket">
										{[...Array(Math.min(15, maxGroupSize))].map((_, i) => (
											<li key={i+1}>
												<a 
							...
							<div className="dropdown-quantity">
								<div className="dropdown">
									...
									<ul className="dropdown-menu dropdown-menu-light" aria-labelledby="dropdownTicketChildren">
										{[...Array(Math.min(10, maxGroupSize))].map((_, i) => (
											<li key={i}>
												<a 
						...
						{isGroupSizeExceeded && (
							<div className="alert alert-warning mt-2 mb-0" style={{fontSize: '12px', padding: '8px'}}>
								Group size exceeds maximum capacity of {maxGroupSize} people
							</div>
						)}
					</div>
				</div>
				...
				<div className="box-button-book"> 
					<button 
						...
						disabled={isGroupSizeExceeded || !selectedDate || totalParticipants === 0}
						style={{opacity: isGroupSizeExceeded || !selectedDate || totalParticipants === 0 ? 0.5 : 1}}
					>
						Book Now
